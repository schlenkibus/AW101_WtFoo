...
void DSPHost::createModule(const std::string &name) {
    auto it = m_moduleFactories.find(name);
    if (it != m_moduleFactories.end()) {
        m_modules.emplace_back(it->second(this));
    }
}
...
namespace algorithm {
    ...
    bool contains(tContainer &container, const T &value) {
...
void DSPHost::cleanDirty() {
    for (auto &c: m_modulesToCreate) {
        createModule(c);
    }

    m_modulesToCreate.clear();
...
void DSPHost::createModuleSafe(const std::string &name) {
    m_modulesToCreate.emplace_back(name);
    setDirty();
}