...
{
 public:
  explicit TestSoundFile(const File& path)
  {
    ...
    if(!m_buffer.loadFromFile(path.getAbsoulutePath()))
      throw std::runtime_error("could not load file: " + path.getAbsoulutePath());

    auto currentMax = 1.0;
    loopRange(m_buffer.getSamples(), m_buffer.getSampleCount(),
              [&currentMax](const sf::Int16& in) { currentMax = std::max<double>(in, currentMax); });
