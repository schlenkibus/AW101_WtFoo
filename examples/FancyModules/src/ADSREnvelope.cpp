...
template <class T> inline void checkTransition(T &segment, ADSREnvelope::State &memberSegmentState)
{
...
ADSREnvelope::ADSREnvelope(DSPHost *h)
    : DSPModule { h }
{
  setValue<Idle>(0.0);
  setValue<Attack>(1.0);
  ...
  createParameter("DecayValue", 0.8, 0, 1.0);
...
bool ADSREnvelope::running()
{
  return currentSegment != Idle;
}
