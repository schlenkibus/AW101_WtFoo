...
Directory::Directory(const tPath &path) : File{path} {
  for(auto& entry: std::experimental::filesystem::directory_iterator(path)) {
    if(entry.symlink_status().type() == std::experimental::filesystem::file_type::directory) {
      m_directorys.emplace_back(this, entry.path());
    } else if(entry.symlink_status().type() == std::experimental::filesystem::file_type::regular) {
...
const Directory *Directory::getParent() const {
  return m_parent;
}
